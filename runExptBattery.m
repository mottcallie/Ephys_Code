...
elseif (strcmpi(newCell, 'n'))
    ...
    switch batterySelect
        case 0
            disp('Battery selection: pursuit varying object shape.')
            pattSelect = [2 4];
            funcSelect = [5 6];
        case 1
            disp('Battery selection: pursuit varying target speed.')
            pattSelect = [2 2 2];
            funcSelect = [3 5 7];
        case 2
            disp('Battery selection: pursuit varying target sweep range.')
            pattSelect = [2 2 2];
            funcSelect = [5 5 11];
    end
    ...
    while 1 %repeat expt until user breaks
        for te = 1:trialPerExpt
            ...
            for rp = randperm(length(pattSelect))
                ...
                disp(['[NOTICE] Starting ' exptTypeName ' experiment selection ' varN '...'])
                [rawData, inputParams, rawOutput] = exptFn(settings,trialDuration,pattN,funcN);
