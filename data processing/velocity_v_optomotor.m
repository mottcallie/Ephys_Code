...
% rotational sweeps during an optomotor assay. plots left and right sweeps
% separately.
%
% OUTPUTS:
...
function [mean_vR,mean_vL,pos_sweep] = velocity_v_optomotor(int_panelps, int_forward,int_angular,int_sideway,int_time,optPlot)
%% pull directional velocity for each sweep

% remove panel data noise
int_panelps_r = round(int_panelps); %round
farRight = median(max(int_panelps_r));
farLeft = median(min(int_panelps_r));

for t=1
    % find indices when pattern at rest on either side
    restingLeft = find(int_panelps_r(:,t)==farLeft);
    restingRight = find(int_panelps_r(:,t)==farRight);
end
...
farRight = median(max(int_panelps_r));
farRight_idx = find(int_panelps_r(:,1)==farRight);
farLeft = median(min(int_panelps_r));
