...
%initialize
nPat = length(allPattNames); %number of pat types
...
for e = 1:length(allFiles)
    ...
    for n = 1:length(allPattNames)
        trial_idx(n) = contains(trialName,allPattNames{n});
    end
...
for tr = 1:nPat
    ...
    pk_times = sort([expttime(pk_idx) ; expttime(pkinv_idx)]');
    
    % plot angular velocity trials
    for r = 1:totTrials
        subplot(totTrials+1,1,r+1);
        plot(expttime, allAngular{tr}(:,r), 'Color','#0072BD')
        
        xlim(xrange)
        for s = pk_times
            xline(s,':','Color','k') %line at switch
        end
    ...
    sgtitle([strrep(filebase,'_',' ') ' - ' strrep(allPattNames{tr},'_',' ') ': angular velocity trials'])
    %save
    trial_filebase = [filebase '_' allPattNames{tr} '_plot_atrials.png'];
    saveas(gcf,trial_filebase);
    copyfile(trial_filebase, dropboxfolder,'f');
...
for tr = 1:nPat
    ...
    for r = 1:size(allForward{tr},2)
        plot(expttime, allForward{tr}(:,r), 'Color','#D95319')
        hold on
    end
    hold off
    xlim(xrange)
    ...
    for r = 1:size(allSideways{tr},2)
        plot(expttime, allSideways{tr}(:,r), 'Color','#7E2F8E')
        hold on
    end
    hold off
    xlim(xrange)
    ...
    sgtitle([strrep(filebase,'_',' ') ' - ' strrep(allPattNames{tr},'_',' ')])
    %save
    sum_filebase = [filebase '_' allPattNames{tr} '_plot_summary.png'];
...
%     pk_times = sort([time(pk_idx) ; time(pkinv_idx)]');
    ...
    sgtitle([strrep(filebase,'_',' ') ' - ' strrep(allPattNames{tr},'_',' ') ': sliding correlation'])
    %save
    corr_filebase = [filebase '_' allPattNames{tr} '_plot_corr.png'];
...
for vo = 1:nPat
    % forward row 1
    subplot(3,nPat,vo)
    velocity_v_panelpos(allPanelPos{vo},allForward{vo},allPattNames{vo})
    if vo == 1
        ylabel('fwd vel (mm/s)')
    end
