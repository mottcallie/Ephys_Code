% velocity_v_panelpos
% function for generating summary plot of velocity v panel position
%
% INPUTS:
% allPanelPos - panel data
% allVelocity - velocity data, either forward sideways or angular
% trialtitle - what to call subplot
%
% ORIGINAL: 12/21/2021 - MC
%

function velocity_v_panelpos(allPanelPos,allVelocity,trialtitle)
...
if pk_idx(1) < pkinv_idx(1) %if right first
    ...
    for k = 1:length(pk_idx)
        ...
        if k == 1
            righttrials_v{k} = reshapeVel(1:pk_idx(k));
            lefttrials_v{k} = reshapeVel(pk_idx(k):pkinv_idx(k));
        ...
        elseif k == length(pk_idx)
            righttrials_v{k} = reshapeVel(pk_idx(k)-pk_dist:pk_idx(k));
            lefttrials_v{k} = reshapeVel(pk_idx(k):pkinv_idx(k));
...
for p = unique(reshapePos)'
    ...
    data_r(i,2) = nanmean(reshapeVel(pos_right)); %mean spikerate
    data_r(i,3) = nanstd(reshapeVel(pos_right)); %standard deviation spikerate
    
    data_l(i,1) = p - midpoint;
    data_l(i,2) = nanmean(reshapeVel(pos_left)); %mean spikerate
    data_l(i,3) = nanstd(reshapeVel(pos_left)); %standard deviation spikerate
    i = i+1;
end
...
if ~contains(trialtitle,'fwd')
    minval = min([data_r(:,2) data_l(:,2)],[],'all');
    maxval = max([data_r(:,2) data_l(:,2)],[],'all');
    turncenter = (maxval-minval)/2 + minval;
    yline(turncenter,':','Color','k')
end
