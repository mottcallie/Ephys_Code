% spike_v_panelpos
% function for generating summary plot of spikerate v panel position
%
% INPUTS:
% allPanelPos - panel data
% allSpikeRate - spikerate data
% trialtitle - what to call subplot
%
% ORIGINAL: 05/25/2022 - MC
%

function spike_v_panelpos(allPanelPos,allSpikeRate,trialtitle)
...
if pk_idx(1) < pkinv_idx(1) %if right first
    ...
    for k = 1:length(pk_idx)
        ...
        if k == 1
            righttrials_v{k} = reshapeSR(1:pk_idx(k));
            lefttrials_v{k} = reshapeSR(pk_idx(k):pkinv_idx(k));
        ...
        elseif k == length(pk_idx)
            righttrials_v{k} = reshapeSR(pk_idx(k)-pk_dist:pk_idx(k));
            lefttrials_v{k} = reshapeSR(pk_idx(k):pkinv_idx(k));
...
for p = unique(reshapePos)'
    ...
    data_r(i,2) = nanmean(reshapeSR(pos_right)); %mean spikerate
    data_r(i,3) = nanstd(reshapeSR(pos_right)); %standard deviation spikerate
    
    data_l(i,1) = p - midpoint;
    data_l(i,2) = nanmean(reshapeSR(pos_left)); %mean spikerate
    data_l(i,3) = nanstd(reshapeSR(pos_left)); %standard deviation spikerate
    i = i+1;
end
